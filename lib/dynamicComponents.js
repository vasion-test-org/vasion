...
export const createDynamicComponents = () => {
  ...
  Object.entries(CoreComponents).forEach(([name, importFn]) => {
    components[name] = dynamic(importFn, {
      loading: () => <div style={{ minHeight: '100px' }} />,
    });
  });

  // Heavy components - lazy load with better loading states
  Object.entries(HeavyComponents).forEach(([name, importFn]) => {
    components[name] = dynamic(importFn, {
  ...
  // Centered section components
  Object.entries(CenteredSectionComponents).forEach(([name, importFn]) => {
    components[name] = dynamic(importFn, {
  ...
  // Copy components
  Object.entries(CopyComponents).forEach(([name, importFn]) => {
    components[name] = dynamic(importFn, {
  ...
  // Utility components
  Object.entries(UtilityComponents).forEach(([name, importFn]) => {
    components[name] = dynamic(importFn, {
      loading: () => <div style={{ minHeight: '100px' }} />,
    });
  });
