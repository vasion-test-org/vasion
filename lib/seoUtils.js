...
/**
 * Checks if a story translation is published in Storyblok
 * @param {string} slug - The story slug
 * @param {string} locale - The locale to check
 * @returns {Promise<boolean>} - True if the translation is published
 */
export async function isStoryTranslationPublished(slug, locale) {
 ...
 * @returns {Promise<Object>} - Object with language codes as keys and URLs as values
 */
export async function buildAlternateLanguageUrls(
  baseUrl,
  currentLocale,
  storySlug,
  slugArray,
  ...
  if (currentLocale === 'en') {
    ...
    const path = slugArray?.length === 1 ? currentLocale : `${currentLocale}/${storySlug}`;
    alternateLinks[currentLocale] = buildCanonicalUrl(baseUrl, path);
  }

  // Add other language versions if they exist and are published
  if (translatedSlugs) {
    for (const translation of translatedSlugs) {
      // Check if this translation is published
      const isPublished = await isStoryTranslationPublished(storySlug, translation.lang);
  ...
  // Always ensure English version is included for non-English pages (if published)
  if (currentLocale !== 'en') {
    // Check if English version is published
    const isEnglishPublished = await isStoryTranslationPublished(storySlug, 'en');

    if (isEnglishPublished) {
      // Try to get the English version from translated_slugs first
      let englishUrl = null;
      if (translatedSlugs) {
        const englishTranslation = translatedSlugs.find((t) => t.lang === 'en');
        if (englishTranslation) {
          const path = englishTranslation.path === 'home' ? '' : englishTranslation.path;
