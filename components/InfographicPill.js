  ...
  const imageSrc =
    blok.media && blok.media.length > 0 && blok.media[0]?.media?.length > 0
      ? useMedia(
          ...
          blok.media[0]?.media[0]?.filename,
          blok.media[0]?.media[1]?.filename || blok.media[0]?.media[0]?.filename,
          blok.media[0]?.media[2]?.filename || blok.media[0]?.media[0]?.filename
        )
      : null;
  return (
...
const PillWrapper = styled.div`
  ...
  ${media.mobile} {
    width: 89.167vw;
    flex-direction: ${(props) => (props?.flipcontent ? 'column-reverse' : 'column')};
    gap: 8.333vw;
