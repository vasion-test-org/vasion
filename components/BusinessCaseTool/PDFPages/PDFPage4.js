...
const PDFPage4 = ({ savingsFormData, currency }) => {
  const {
    printerQty,
    pageCost,
    pagesPrinted,
  ...
  const year1ProductivitySavings = {
    lowSavings:
      ((printerQty * 15) / 60) * 1 +
      ((tickets * 0.4 * resolveTickets * 0.3) / 60) * 1,
    midSavings:
      ((printerQty * 15) / 60) * 1 +
      ((tickets * 0.4 * resolveTickets * 0.5) / 60) * 1,
    highSavings:
      ((printerQty * 15) / 60) * 1 +
      ((tickets * 0.4 * resolveTickets * 0.9) / 60) * 1,
  };

  const year3ProductivitySavings = {
    lowSavings:
      ((printerQty * 15) / 60) * 3 +
      ((tickets * 0.4 * resolveTickets * 0.3) / 60) * 3,
    midSavings:
      ((printerQty * 15) / 60) * 3 +
      ((tickets * 0.4 * resolveTickets * 0.5) / 60) * 3,
    highSavings:
      ((printerQty * 15) / 60) * 3 +
      ((tickets * 0.4 * resolveTickets * 0.9) / 60) * 3,
  };

  const year5ProductivitySavings = {
    lowSavings:
      ((printerQty * 15) / 60) * 5 +
      ((tickets * 0.4 * resolveTickets * 0.3) / 60) * 5,
    midSavings:
      ((printerQty * 15) / 60) * 5 +
      ((tickets * 0.4 * resolveTickets * 0.5) / 60) * 5,
    highSavings:
      ((printerQty * 15) / 60) * 5 +
      ((tickets * 0.4 * resolveTickets * 0.9) / 60) * 5,
  };
  ...
  return (
    <PageWrapper>
      ...
      <Text>
        ...
        by removing legacy print infrastructure, boosting productivity, and
        rationalizing your printer fleet. These numbers vary greatly based on
        your approach and the implementation of the cost-saving features we
        provide.
      </Text>
      <TabTable>
        <TabTableHeaderRow>
        ...
        <TableRow>
          ...
          <TableData>{currency}{formatNumber(year1Savings?.lowSavings)}</TableData>
          <TableData>{currency}{formatNumber(year3Savings?.lowSavings)}</TableData>
          <TableData>{currency}{formatNumber(year5Savings?.lowSavings)}</TableData>
        </TableRow>
        <TableRow>
          <TableData>Mid</TableData>
          <TableData>{currency}{formatNumber(year1Savings?.midSavings)}</TableData>
          <TableData>{currency}{formatNumber(year3Savings?.midSavings)}</TableData>
          <TableData>{currency}{formatNumber(year5Savings?.midSavings)}</TableData>
        </TableRow>
        <TableRow>
          <TableData>High</TableData>
          <TableData>{currency}{formatNumber(year1Savings?.highSavings)}</TableData>
          <TableData>{currency}{formatNumber(year3Savings?.highSavings)}</TableData>
          <TableData>{currency}{formatNumber(year5Savings?.highSavings)}</TableData>
        </TableRow>
      </TabTable>
      <BodyHeadline>Your Total Projected Productivity Savings</BodyHeadline>
      <Text>
        You can achieve the following productivity savings by eliminating
        helpdesk tickets and automating print driver updates.
      </Text>
      ...
      <TabTable>
        ...
        <TableRow>
          <TableData>Low</TableData>
          <TableData>
            {formatNumber(year1ProductivitySavings?.lowSavings)}
          </TableData>
          <TableData>
            {formatNumber(year3ProductivitySavings?.lowSavings)}
          </TableData>
          <TableData>
            {formatNumber(year5ProductivitySavings?.lowSavings)}
          </TableData>
        </TableRow>
        <TableRow>
          <TableData>Mid</TableData>
          <TableData>
            {formatNumber(year1ProductivitySavings?.midSavings)}
          </TableData>
          <TableData>
            {formatNumber(year3ProductivitySavings?.midSavings)}
          </TableData>
          <TableData>
            {formatNumber(year5ProductivitySavings?.midSavings)}
          </TableData>
        </TableRow>
        <TableRow>
          <TableData>High</TableData>
          <TableData>
            {formatNumber(year1ProductivitySavings?.highSavings)}
          </TableData>
          <TableData>
            {formatNumber(year3ProductivitySavings?.highSavings)}
          </TableData>
          <TableData>
            {formatNumber(year5ProductivitySavings?.highSavings)}
          </TableData>
        </TableRow>
      </TabTable>

      <BodyHeadline>
        Your Total Projected Carbon Emissions Reduction
      </BodyHeadline>
      <Text>
        You can reduce carbon emissions by eliminating print servers and
        implementing eco-friendly print features like pull printing to reduce
        print volumes.
      </Text>
      <TabTable>
        <TabTableHeaderRow>
