...
const G2BadgesMobile = ({ badges }) => {
  ...
  const allBadges = badges.map((badges, index) => (
    <BadgeDiv className="badges">
      <BadgeImage src={badges.badge.sourceUrl} alt={badges?.badge?.altText} />
      <BadgeContent>
        <BadgeTitle>{badges.badgeName}</BadgeTitle>
        <BadgeCopy>{badges.badgeCopy}</BadgeCopy>
      </BadgeContent>
    </BadgeDiv>
  ));
